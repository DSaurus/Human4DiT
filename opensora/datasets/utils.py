...
def save_sample(x, fps=8, save_path=None, normalize=True, value_range=(-1, 1)):
    """
    Args:
        x (Tensor): shape [C, T, H, W] or [C, V, T, H, W]
    """
    if x.ndim == 5:
        x = rearrange(x, "c v t h w -> c t h (v w)")
    assert x.ndim == 4
    ...
    if x.shape[1] == 1:  # T = 1: save as image
        ...
        x_np = x.numpy()
        num_frames, height, width, _ = x_np.shape
        # Initialize a VideoWriter object
        fourcc = cv2.VideoWriter_fourcc(*'XVID')  # or use 'XVID'
        out = cv2.VideoWriter(save_path, fourcc, fps, (width, height))

        # Iterate over the frames in the numpy array
        for i in range(num_frames):
            # Convert the frame to BGR format
            frame_bgr = cv2.cvtColor(x_np[i], cv2.COLOR_RGB2BGR)
