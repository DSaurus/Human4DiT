...
class T5Encoder:
    ...
    def encode(self, text):
        caption_embs, emb_masks = torch.zeros(len(text), 120, 4096).cuda(), torch.zeros(len(text), 120).cuda().long()
        emb_masks[:, :4] = 1
        caption_embs = caption_embs[:, None]
        return dict(y=caption_embs, mask=emb_masks)
        caption_embs, emb_masks = self.t5.get_text_embeddings(text)
        print(caption_embs.shape, emb_masks.shape)
        print(emb_masks)
        print(torch.sum(emb_masks))
        exit(0)
        caption_embs = caption_embs[:, None]
        return dict(y=caption_embs, mask=emb_masks)
