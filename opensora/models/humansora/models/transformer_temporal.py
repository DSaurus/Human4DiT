...
class TransformerTemporal(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None):
        _, num_channels, v, f, height, width = hidden_states.shape
        hidden_states = rearrange(hidden_states, "b c v f h w -> (b v f) c h w")

        skip = hidden_states

        hidden_states = self.norm(hidden_states)
        hidden_states = rearrange(hidden_states, "bvf c h w -> bvf (h w) c")
        ...
        hidden_states = rearrange(hidden_states, "bvf (h w) c -> bvf c h w", h=height, w=width).contiguous()

        output = hidden_states + skip
        output = rearrange(output, "(b v f) c h w -> b c v f h w", f=f, v=v)
