...
def get_down_block(
    ...
    if down_block_type == "DownBlock4D":
        return DownBlock4D(
    ...
    elif down_block_type == "CrossAttnDownBlock4D":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for CrossAttnDownBlock4D")
        return CrossAttnDownBlock4D(
            num_layers=num_layers,
            in_channels=in_channels,
            out_channels=out_channels,
...
def get_up_block(
    ...
    elif up_block_type == "CrossAttnUpBlock4D":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for CrossAttnUpBlock4D")
        return CrossAttnUpBlock4D(
            num_layers=num_layers,
            in_channels=in_channels,
            transformer_layers_per_block=transformer_layers_per_block,
            out_channels=out_channels,
...
class UNetMidBlock4DCrossAttn(nn.Module):
    def __init__(
            self,
            in_channels: int,
            temb_channels: int,
    ...
    def forward(self, hidden_states, temb=None, encoder_hidden_states=None, attention_mask=None,
        ...
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            hidden_states = attn(hidden_states, encoder_hidden_states=encoder_hidden_states, ref_feat=ref_feat).sample
...
class CrossAttnDownBlock4D(nn.Module):
    def __init__(
            self,
            in_channels: int,
            out_channels: int,
    ...
    def forward(self, hidden_states, temb=None, encoder_hidden_states=None, attention_mask=None,
        ...
        # print(hidden_states.shape)  torch.Size([1, 320, 1, 24, 48, 48])
        # exit(0)
        ...
        for resnet, attn, temporal_attention \
            ...
            if self.training and self.gradient_checkpointing:
                ...
                # print(hidden_states.shape) torch.Size([1, 320, 1, 24, 48, 48])
                hidden_states = resnet(hidden_states, temb)
                # print(hidden_states.shape) torch.Size([1, 640, 1, 24, 48, 48])
                # exit(0)
...
class CrossAttnUpBlock4D(nn.Module):
    def __init__(
            self,
            in_channels: int,
            out_channels: int,
        ...
        if add_upsample:
            self.upsamplers = nn.ModuleList([Upsample4D(out_channels, use_conv=True, out_channels=out_channels)])
        else:
