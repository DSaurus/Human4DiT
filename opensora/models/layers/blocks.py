...
class CondConvNet(nn.Module):
    def __init__(
        ...
        self.net_modules = []
        first_conv = nn.Conv2d(in_chans, base_channel, kernel_size=3, stride=2, padding=1)
        self.net_modules.append(first_conv)
        norm = nn.GroupNorm(4, base_channel)
        self.net_modules.append(norm)
        activation = nn.LeakyReLU(0.2)
        self.net_modules.append(activation)
        for i in range(downsample_times-1):
            conv = nn.Conv2d(base_channel, base_channel*2, kernel_size=3, stride=2, padding=1)
            base_channel *= 2
            self.net_modules.append(conv)
            norm = nn.GroupNorm(4, base_channel)
            self.net_modules.append(norm)
            activation = nn.LeakyReLU(0.2)
            self.net_modules.append(activation)
        final_conv = nn.Conv2d(base_channel, embed_dim, kernel_size=1, stride=1, padding=0)
        self.net_modules.append(final_conv)
        self.net_modules = nn.Sequential(*self.net_modules)
    ...
    def forward(self, x):
        ...
        x = rearrange(x, "(B D) C H W -> B C D H W", D=D)
        if self.flatten:
